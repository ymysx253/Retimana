...
## 画面構成
1. ページタイトル「地方を選ぶ」
2. 日本地図（SVG + 画像 + 地方名ラベル）
3. **都道府県リスト**（地方ごとにグループ化、全47都道府県）
   - 各都道府県はピル型ボタン
   - データのある都道府県のみ有効、それ以外は disabled + 「Coming Soon」バッジ
   - MVP 段階では京都府のみ有効
   - リストが長くなったらページ全体がスクロール

## 画面・UI（対応するファイル）
- [Retimana/Components/Pages/AreaSelect.razor](../../src/Retimana.Web/Components/Pages/AreaSelect.razor) — マークアップ（`/area` ルート）
- [Retimana/Components/Pages/AreaSelect.razor.css](../../src/Retimana.Web/Components/Pages/AreaSelect.razor.css) — スコープ付きCSS
...
### 地図表示（SVG + 画像 + 地方名ラベル）
- 元デザイン: [docs/design/areaselect/japan_map.png](../design/areaselect/japan_map.png)
- 採用アセット: [Retimana/wwwroot/images/japan_map.png](../../src/Retimana.Web/wwwroot/images/japan_map.png)（1536×1024）
...
対策: `.area-label` のスタイルはグローバルCSSの [Retimana/wwwroot/app.css](../../src/Retimana.Web/wwwroot/app.css) に置く。コンポーネントスコープ外なので、コンポーネント側の css ファイルに書く際は注意。
- 構造:
...
| ファイル | 役割 |
|----------|------|
| [Components/Pages/AreaSelect.razor](../../src/Retimana.Web/Components/Pages/AreaSelect.razor) | `/area` ルートのページコンポーネント |
| [Components/Pages/AreaSelect.razor.css](../../src/Retimana.Web/Components/Pages/AreaSelect.razor.css) | スコープ付きCSS（レイアウト・タイトル・地図画像のサイズ） |
| [wwwroot/images/japan_map.png](../../src/Retimana.Web/wwwroot/images/japan_map.png) | 日本地図画像 |
...
- 2026-05-20: タイトルと地図alt textを多言語化（`IStringLocalizer<SharedResources>` 経由、`AreaSelect.Title` / `AreaSelect.MapAlt`）。詳細は [i18n.md](i18n.md) 参照
...
- 2026-05-24: 日本地図画像を新版（`japan_map.png`、より写実的な地図イラスト）に差し替え。旧 `japanmap.png` / `japanmap_transparent.png` を破棄
- 2026-05-24: 地図の下に全47都道府県のリストを地方ごとにグループ化して追加。データがあるのは京都府のみ（他は disabled + Coming Soon バッジ）。多言語対応（4言語×47件 = 188翻訳追加）
